...
auditLogSchema.statics.findByAction = function (action, days = 30, limit = 100) {
  ...
  return this.find({ action, createdAt: { $gte: startDate } })
    .sort({ createdAt: -1 })
    .limit(limit)
    .populate("adminId", "username email");
};
  ...
  return this.aggregate([
    { $match: { createdAt: { $gte: startDate } } },
    {
      $group: {
        ...
        successRate: { $avg: { $cond: ["$result.success", 1, 0] } },
      },
    },
    {
      $lookup: {
...
auditLogSchema.methods.flag = function (reason = "") {
  this.flagged = true;
  if (reason) this.details.flagReason = reason;
  return this.save();
};

auditLogSchema.methods.unflag = function () {
  this.flagged = false;
  if (this.details.flagReason) delete this.details.flagReason;
  return this.save();
};
...
auditLogSchema.pre("save", function (next) {
  ...
  if (this.action && this.tags.length === 0) {
    this.tags = [this.action.split("_")[0]];
  }
