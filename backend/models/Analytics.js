...
analyticsSchema.statics = {
  ...
  async getLatest(period = "day", limit = 10) {
    return this.find({ period }).sort({ startTime: -1 }).limit(limit);
  },
...
analyticsSchema.methods = {
  ...
  async calculateGrowthRate() {
    const previousPeriod = await this.constructor
      .findOne({
        period: this.period,
  ...
  getTopPlatform() {
    if (!this.platformStats?.length) return null;
  ...
  getTopFlagReason() {
    if (!this.flagReasonStats?.length) return null;
